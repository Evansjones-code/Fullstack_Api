...
async def lifespan(_app: FastAPI):
    ...
    """
    yield
    # Shutdown: Ensure the database engine is disposed of cleanly
    await engine.dispose()

# redirect_slashes=True ensures /api/users/ redirects to /api/users
...
# Register routers with clear API prefixes
...
async def validation_exception_handler(request: Request, exception: RequestValidationError):
    ...
    return templates.TemplateResponse(
        request,
        "error.html",
        {"status_code": 422, "message": "Invalid input provided."},
        status_code=422,
    )
